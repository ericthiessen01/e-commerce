...
export default function CartItem() {
    ...
    const cartItemsHtml = cartItems.map(obj => {
        if(obj.quantity > 0) {
            return(
                <div key={obj.id} className=' bg-white p-4 m-4 grid grid-cols-5 shadow' >
                    <img src={obj.image} alt={obj.title} className='p-3 max-h-48 self-center justify-self-center col-span-1'/>
                    <div className='col-start-2 col-span-3 lg:col-span-2 flex flex-col justify-between items-start'>
