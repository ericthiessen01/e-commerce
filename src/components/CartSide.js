...
export default function CartSide() {
  ...
  const sideCartItemsHtml = cartItems.map(obj => {
      if(obj.quantity > 0) {
          return(
              <div key={obj.id} className='w-11/12 my-2 flex bg-white p-2 mx-auto' >
  ...
  return (
      <div className='w-1/4 bg-stone-50 h-[90vh] fixed right-0 shadow-lg overflow-y-auto p-2'>
        {sideCartItemsHtml}
        {totalCartItems > 0 ? 
        <div className='link text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-lg px-5 py-2.5 text-center mx-auto my-2 w-3/5' onClick={() => {navigate("/e-commerce/Cart")}} >Check-out</div> :
